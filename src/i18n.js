export const translations = {
  en: {
    appTitle: "Recipe Calculator",
    appSubtitle: "Smart baking calculator with business insights",
    darkMode: "Dark",
    lightMode: "Light",
    settings: "Settings",
    numberOfBatches: "Number of Batches",
    ingredients: "Ingredients",
    ingredient: "Ingredient",
    grams: "Grams",
    price: "Price",
    totalCost: "Total Cost",
    totalCalories: "Total Calories",
    editPrices: "Edit Prices",
    exportPDF: "Export PDF",
    share: "Share",
    copied: "Copied!",
    copyClipboard: "Copy to Clipboard",
    sendWhatsApp: "Send via WhatsApp",
    editIngredientPrices: "Edit Ingredient Prices",
    pricePerGram: "Price per gram (Rp). Leave blank to use default.",
    defaultPrice: "Default:",
    resetAll: "Reset All",
    done: "Done",
    businessAnalysis: "Business Analysis",
    sellingPrice: "Selling Price / jar (Rp)",
    yieldBatch: "Total Batch Weight (gram)",
    cogsPerPcs: "COGS / jar",
    totalRevenue: "Total Revenue",
    totalHpp: "Total COGS",
    profitLoss: "Profit / Loss",
    margin: "Margin",
    steps: "Steps",
    progress: "Progress",
    prev: "Prev",
    next: "Next",
    markDone: "Done",
    reset: "Reset",
    wipTitle: "Steps under development.",
    wipSub: "Check back soon!",
    allDone: "All steps complete! Your",
    isReady: "is ready!",
    recipeNotes: "Recipe Notes",
    notesPlaceholder: "Add your notes, tips, or modifications here...",
    notesSaved: "Saved!",
    notesHint: "Notes are saved automatically.",
    flour: "Wheat Flour",
    butter: "Butter",
    wisman: "Wisman",
    powdered_sugar: "Powdered Sugar",
    cornstarch: "Cornstarch",
    milk_powder: "Milk Powder",
    cheese_edam: "Edam Cheese",
    cheese_cheddar: "Cheddar Cheese",
    cashew: "Cashew Nuts",
    chocolate_block: "Chocolate Block",
    cocoa_powder: "Cocoa Powder",
    cream_of_tartar: "Cream of Tartar",
    lemon_juice: "Lemon Juice",
    coconut_milk: "Coconut Milk",
    egg_yolk: "Egg Yolk",
    sago_flour: "Sago Flour",
    nastar: "Nastar",
    kastengel: "Kastengel",
    putri: "Putri Salju",
    sagu: "Sagu Keju",
    kompies: "Kompies",
    timerStart: "Start",
    timerPause: "Pause",
    timerReset: "Reset",
    timerDone: "Done!",
    timerMin: "min",
    history: "History",
    historySub: "Your recent calculation results.",
    historyEmpty: "No history yet. Start calculating!",
    historyClear: "Clear All",
    costBreakdown: "Cost Breakdown",
    targetMarginTitle: "Auto Selling Price Calculator",
    targetMarginLabel: "Target Margin",
    suggestedPrice: "Suggested Price / jar",
    applyPrice: "Apply This Price",
    printLabel: "Print Label",
    printLabelSub: "Fill in details below. 6 labels will be printed per page.",
    jarSize: "Jar Size",
    packagingCost: "Packaging / jar",
    totalPackagingCost: "Total Packaging Cost",
    totalJars: "Total Jars",
    jar: "jar",
    jar250: "250 gram jar",
    jar500: "500 gram jar",
    jar1000: "1000 gram jar",
    brandName: "Brand Name",
    weightGram: "Weight (gram)",
    phoneNum: "Phone Number",
    expDays: "Expiry (days from today)",
    customNote: "Custom Note",
    printNow: "Print Now",
    shareTitle: "Recipe Calculator",
    shareBatch: "batch",
    shareIngredients: "Ingredients",
    shareBusiness: "Business Analysis",
    shareGenerated: "Generated by Recipe Calculator",
  },

  id: {
    appTitle: "Kalkulator Resep",
    appSubtitle: "Kalkulator kue pintar dengan analisis bisnis",
    darkMode: "Gelap",
    lightMode: "Terang",
    settings: "Pengaturan",
    numberOfBatches: "Jumlah Batch",
    ingredients: "Bahan-Bahan",
    ingredient: "Bahan",
    grams: "Gram",
    price: "Harga",
    totalCost: "Total Modal",
    totalCalories: "Total Kalori",
    editPrices: "Edit Harga",
    exportPDF: "Ekspor PDF",
    share: "Bagikan",
    copied: "Tersalin!",
    copyClipboard: "Salin ke Clipboard",
    sendWhatsApp: "Kirim via WhatsApp",
    editIngredientPrices: "Edit Harga Bahan",
    pricePerGram: "Harga per gram (Rp). Kosongkan untuk pakai harga default.",
    defaultPrice: "Default:",
    resetAll: "Reset Semua",
    done: "Selesai",
    businessAnalysis: "Analisis Bisnis",
    sellingPrice: "Harga Jual / toples (Rp)",
    yieldBatch: "Total Hasil Batch (gram)",
    cogsPerPcs: "HPP / toples",
    totalRevenue: "Total Pendapatan",
    totalHpp: "Total HPP",
    profitLoss: "Laba / Rugi",
    margin: "Margin",
    steps: "Langkah-Langkah",
    progress: "Progress",
    prev: "Sebelumnya",
    next: "Selanjutnya",
    markDone: "Selesai",
    reset: "Reset",
    wipTitle: "Langkah-langkah sedang dikembangkan.",
    wipSub: "Segera hadir!",
    allDone: "Semua langkah selesai! Kue",
    isReady: "kamu sudah siap!",
    recipeNotes: "Catatan Resep",
    notesPlaceholder: "Tambahkan catatan, tips, atau modifikasi resep di sini...",
    notesSaved: "Tersimpan!",
    notesHint: "Catatan disimpan otomatis.",
    flour: "Tepung Terigu",
    butter: "Mentega",
    wisman: "Wisman",
    powdered_sugar: "Gula Halus",
    cornstarch: "Maizena",
    milk_powder: "Susu Bubuk",
    cheese_edam: "Keju Edam",
    cheese_cheddar: "Keju Cheddar",
    cashew: "Kacang Mete",
    chocolate_block: "Cokelat Blok",
    cocoa_powder: "Bubuk Kakao",
    cream_of_tartar: "Cream of Tartar",
    lemon_juice: "Air Lemon",
    coconut_milk: "Santan",
    egg_yolk: "Kuning Telur",
    sago_flour: "Tepung Sagu",
    nastar: "Nastar",
    kastengel: "Kastengel",
    putri: "Putri Salju",
    sagu: "Sagu Keju",
    kompies: "Kompies",
    timerStart: "Mulai",
    timerPause: "Jeda",
    timerReset: "Ulang",
    timerDone: "Selesai!",
    timerMin: "menit",
    history: "Riwayat",
    historySub: "Hasil kalkulasi terakhirmu.",
    historyEmpty: "Belum ada riwayat. Mulai kalkulasi!",
    historyClear: "Hapus Semua",
    costBreakdown: "Rincian Biaya",
    targetMarginTitle: "Kalkulator Harga Jual Otomatis",
    targetMarginLabel: "Target Margin",
    suggestedPrice: "Harga Jual Saran / toples",
    applyPrice: "Pakai Harga Ini",
    printLabel: "Cetak Label",
    printLabelSub: "Isi detail di bawah. 6 label akan dicetak per halaman.",
    jarSize: "Ukuran Toples",
    packagingCost: "Packaging / toples",
    totalPackagingCost: "Total Biaya Packaging",
    totalJars: "Jumlah Toples",
    jar: "toples",
    jar250: "Toples 250 gram",
    jar500: "Toples 500 gram",
    jar1000: "Toples 1000 gram",
    brandName: "Nama Brand",
    weightGram: "Berat (gram)",
    phoneNum: "Nomor Telepon",
    expDays: "Kedaluwarsa (hari dari sekarang)",
    customNote: "Catatan Khusus",
    printNow: "Cetak Sekarang",
    shareTitle: "Kalkulator Resep",
    shareBatch: "batch",
    shareIngredients: "Bahan-Bahan",
    shareBusiness: "Analisis Bisnis",
    shareGenerated: "Dibuat dengan Kalkulator Resep",
  },
};

export function t(lang, key) {
  return translations[lang]?.[key] ?? translations.en[key] ?? key;
}
